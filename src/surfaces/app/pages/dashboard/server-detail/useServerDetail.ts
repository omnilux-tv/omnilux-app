...
export const useServerDetail = () => {
  ...
  const detailQuery = useQuery({
    queryKey,
    queryFn: async () => {
      const data = await invokeCloudFunction<ServerDetailResponse>(
        "get-server-detail",
        { body: { serverId } }
      );
  ...
  const relayCondition = server
    ? deriveRelayCondition({
        ...
        entitled: isSelfHosted
          ? (accessProfile?.relayRemoteAccessEntitled ?? null)
          : (accessProfile?.managedMediaEntitled ?? null),
      })
    : null;
  const relayTone = relayCondition
    ? getRelayConditionTone(relayCondition)
    : "muted";
  const remoteRelaySupported = server
    ? hasRemoteHttpRelaySupport(server.relay_capabilities)
    : false;
  const relayEntitled = accessProfile?.relayRemoteAccessEntitled === true;
  const relayRemoteSessionsEnabled =
  ...
  const createInvite = useMutation({
    mutationFn: () =>
      invokeCloudAction("create-invite", {
        body: { serverId, role: inviteRole, maxUses: inviteMaxUses },
      }),
    onSuccess: invalidate,
  });
  const revokeInvite = useMutation({
    mutationFn: (inviteId: string) =>
      invokeCloudAction("revoke-server-invite", { body: { inviteId } }),
    onSuccess: invalidate,
  });
  const removeAccess = useMutation({
    mutationFn: (accessId: string) =>
      invokeCloudAction("remove-server-access", { body: { accessId } }),
    onSuccess: invalidate,
  });
  const openRelaySession = useMutation({
    mutationFn: async () => {
      const data = await invokeCloudFunction<CreateRelaySessionResponse>(
        "create-relay-session",
        {
          body: {
            serverId,
            sessionType: "remote-access",
            metadata: { surface: "app-server-detail" },
          },
        }
      );
    ...
    onSuccess: (data) =>
      window.open(
        buildRelaySessionUrl(data.token),
        "_blank",
        "noopener,noreferrer"
      ),
  });
