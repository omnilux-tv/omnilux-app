    ...
    {vm.access && vm.access.length > 0 ? (
      <div className="space-y-2">
        {vm.access.map((access) => (
          <div key={access.id} className="flex items-center justify-between rounded-lg bg-surface/50 px-4 py-2">
            <div>
              <span className="text-sm text-foreground">{access.user_name ?? access.user_email ?? 'Unknown user'}</span>
              {access.user_email && access.user_name && access.user_name !== access.user_email ? (
                <span className="ml-2 text-xs text-muted">{access.user_email}</span>
              ) : null}
              <span className="ml-2 text-xs text-muted capitalize">({access.role})</span>
            </div>
            <button
              type="button"
