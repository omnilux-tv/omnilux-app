...
const invokeProviderWorkspace = <TResponse>(
  operation: string,
  input?: unknown
) =>
  invokeControlFunction<TResponse, unknown>(
    "managed-media-provider-workspace",
    operation,
    input
  );
...
export const useProviderWorkspace = (enabled: boolean) => {
  ...
  const workspacesQuery = useQuery({
    ...
    queryFn: async () => {
      const data =
        await invokeProviderWorkspace<ManagedMediaProviderWorkspacesResponse>(
          "workspace.list"
        );
  ...
  const workspace =
    workspaces.find((entry) => entry.provider.id === selectedProviderId) ??
    workspaces[0] ??
    null;
  const catalogItems = workspace?.catalogItems ?? [];
  const releaseVersions = workspace?.releaseVersions ?? [];
  const selectedReleaseItem =
    catalogItems.find((item) => item.id === release.itemId) ??
    catalogItems[0] ??
    null;
  const releaseVersionNumberValue = Number.parseInt(release.versionNumber, 10);
  const selectedRequestReleaseVersion =
    releaseVersions.find((entry) => entry.id === request.releaseVersionId) ??
    null;
  const selectedRequestItem =
    catalogItems.find(
      (item) =>
        item.id === (selectedRequestReleaseVersion?.itemId ?? request.itemId)
    ) ?? null;
  const requestReleaseOptions = releaseVersions.filter(
    (entry) => !selectedRequestItem || entry.itemId === selectedRequestItem.id
  );
  const selectedDeliveryReleaseVersion =
    releaseVersions.find((entry) => entry.id === delivery.releaseVersionId) ??
    null;
  const selectedDeliveryItem =
    catalogItems.find(
      (item) =>
        item.id === (selectedDeliveryReleaseVersion?.itemId ?? delivery.itemId)
    ) ?? null;
  const deliveryReleaseOptions = releaseVersions.filter(
    (entry) => !selectedDeliveryItem || entry.itemId === selectedDeliveryItem.id
  );

  const capabilities = useMemo(
    () => new Set(workspace?.membership.capabilities ?? []),
    [workspace]
  );
  const providerRequestCapability =
    request.requestType === "support"
      ...
      : request.requestType === "issue_response"
        ...
        : ["rights_review", "takedown", "rollback"].includes(
              request.requestType
            )
  ...
  const submitProviderRequest = useMutation({
    mutationFn: (body: ManagedMediaProviderRequestSubmitRequest) =>
      invokeProviderWorkspace<unknown>("request.submit", body),
    onSuccess: async () => {
  ...
  const submitCatalogItem = useMutation({
    mutationFn: (body: ManagedMediaProviderCatalogItemUpsertRequest) =>
      invokeProviderWorkspace<ManagedMediaProviderCatalogItemResponse>(
        "catalog-item.upsert",
        body
      ),
    onSuccess: async (result) => {
      setMessage(
        `Catalog draft saved: ${result?.item.title ?? catalog.title.trim()}.`
      );
      setCatalog((current) => ({
        ...current,
        externalItemKey: "",
  ...
  const submitReleaseVersion = useMutation({
    mutationFn: (body: ManagedMediaProviderReleaseVersionUpsertRequest) =>
      invokeProviderWorkspace<ManagedMediaProviderReleaseVersionResponse>(
        "release-version.upsert",
        body
      ),
    onSuccess: async (result) => {
      setMessage(
        `Release draft saved: ${result?.releaseVersion.releaseVersionKey ?? release.releaseVersionKey.trim()}.`
      );
      setRelease((current) => ({
        ...current,
        releaseVersionKey: "",
  ...
  const submitAssetDelivery = useMutation({
    mutationFn: (body: ManagedMediaProviderAssetDeliverySubmitRequest) =>
      invokeProviderWorkspace<ManagedMediaProviderAssetDeliveryResponse>(
        "asset-delivery.submit",
        body
      ),
    onSuccess: async (result) => {
      setMessage(
        `Asset delivery submitted: ${result?.assetDelivery.title ?? delivery.title.trim()}.`
      );
      setDelivery((current) => ({
        ...current,
        deliveryKey: "",
  ...
  const submitRightsPolicy = useMutation({
    mutationFn: (body: ManagedMediaProviderRightsPolicyUpsertRequest) =>
      invokeProviderWorkspace<ManagedMediaProviderRightsPolicyResponse>(
        "rights-policy.upsert",
        body
      ),
    onSuccess: async (result) => {
      setMessage(
        `Rights policy draft saved: ${result?.rightsPolicy.policyKey ?? rights.policyKey.trim()}.`
      );
      setRights((current) => ({
        ...current,
        policyKey: "",
  ...
  const exportProviderReport = useMutation({
    mutationFn: async (payload: {
      ...
      const data =
        await invokeProviderWorkspace<ManagedMediaProviderReportExportResponse>(
          "report.export",
          { ...payload, format: "csv", source: "provider-dashboard" }
        );
      if (!data?.csv)
    ...
    onSuccess: (result) => {
      ...
      const reportLabel =
        result.reportType === "settlement-basis"
          ...
          : "item-aggregate";
      downloadTextFile(
        `managed-media-${safeProviderId}-${reportLabel}-${result.generatedAt.slice(0, 10)}.csv`,
        result.csv ?? "",
  ...
  const selectedProviderUnitLabel = workspace
    ? (providerUnitKindLabels[workspace.provider.providerUnitKind ?? "root"] ??
      "Provider unit")
    : null;
  ...
  return {
    ...
    patch: {
      request: (patch: Partial<typeof request>) =>
        setRequest((current) => ({ ...current, ...patch })),
      catalog: (patch: Partial<typeof catalog>) =>
        setCatalog((current) => ({ ...current, ...patch })),
      release: (patch: Partial<typeof release>) =>
        setRelease((current) => ({ ...current, ...patch })),
      delivery: (patch: Partial<typeof delivery>) =>
        setDelivery((current) => ({ ...current, ...patch })),
      rights: (patch: Partial<typeof rights>) =>
        setRights((current) => ({ ...current, ...patch })),
    },
    derived: {
      catalogItems,
      releaseVersions,
...
export type ProviderWorkspaceViewModel = ReturnType<
  typeof useProviderWorkspace
>;
