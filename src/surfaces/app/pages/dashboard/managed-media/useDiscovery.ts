...
export const useDiscovery = ({ enabled, getAccessToken, managedMediaOrigin, setLaunchError }: DiscoveryArgs) => {
  ...
  const cloudDiscoveryQuery = useQuery({
    ...
    queryFn: () => {
      ...
      return invokeCloudFunction<ManagedMediaDiscoveryResponse>('list-managed-media-discovery', { body });
    },
    enabled,
  });

  const sourceItems =
    cloudDiscoveryQuery.data?.discoveryItems && cloudDiscoveryQuery.data.discoveryItems.length > 0
      ? cloudDiscoveryQuery.data.discoveryItems
      : managedMediaFixtureItems;
  const usingCloudDiscovery = sourceItems === cloudDiscoveryQuery.data?.discoveryItems;
  ...
  const detailQuery = useQuery({
    ...
    queryFn: () => {
      if (!selectedDiscoveryItem) throw new Error('Select a managed media item before loading details.');
      ...
      return invokeCloudFunction<ManagedMediaCatalogItemDetailResponse>('get-managed-media-catalog-item', { body });
    },
  ...
  const launchDiscoveryItem = async (item: ManagedDiscoveryItem) => {
    ...
    try {
      ...
      const grantRequest: ManagedMediaPlaybackGrantIssueRequest = usingCloudDiscovery
        ? {
            action,
            media: { releaseVersionId: item.playback.releaseVersionId, assetId: item.playback.assetId },
      ...
      const result = await launchManagedMediaPlayback({ mediaOrigin: managedMediaOrigin, getAccessToken, grantRequest });
      ...
      }).catch((error) => console.warn('Managed media playback grant could not be marked consumed.', error));
      ...
      }).catch((error) => console.warn('Managed media usage event could not be recorded.', error));
      window.location.assign(new URL(result.launch.stream.directFileUrl, managedMediaOrigin).toString());
    } catch (caughtError) {
      setLaunchError(caughtError instanceof Error ? caughtError.message : 'OmniLux Media could not prepare playback for this item.');
    } finally {
