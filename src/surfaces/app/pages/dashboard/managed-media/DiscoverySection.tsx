    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted">
          Signed-in catalog entries use OmniLux Cloud release state when available, with safe fixture previews reserved
          for empty early environments.
        </p>
        {discovery.cloudDiscovery?.result ? (
          <p className="mt-2 text-xs text-muted">
            {numberFormatter.format(discovery.cloudDiscovery.result.returned)} of{' '}
            {numberFormatter.format(discovery.cloudDiscovery.result.totalMatched)} matching cloud items
        ...
        <article key={item.id} className="rounded-xl border border-border bg-surface p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            >
              ...
              {discovery.selectedDiscoveryItemId === item.id ? 'Details shown' : 'View details'}
            </button>
            {item.entitlement.playbackAllowed && item.playback ? (
              ...
              >
                ...
                {discovery.playbackLaunchItemId === item.id ? 'Preparing...' : 'Launch playback'}
              </button>
            ) : null}
          </div>
        </article>
      ...
      <article className="mt-6 rounded-xl border border-border bg-surface p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h3 className="mt-1 text-lg font-bold text-foreground">{discovery.selectedDetailItem.title}</h3>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">{discovery.selectedDetailItem.description}</p>
          </div>
          <button
            type="button"
        ...
        {discovery.isSelectedCatalogItemDetailLoading ? <p className="mt-4 text-sm text-muted">Loading cloud catalog detail...</p> : null}
          ...
          {[
            ...
            [
              ...
              discovery.selectedCatalogItemDetail?.policy.decision ??
                (discovery.selectedDetailItem.entitlement.playbackAllowed ? 'allow' : 'preview-only'),
            ],
          ].map(([label, value]) => (
