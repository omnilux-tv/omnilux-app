    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="text-xs font-semibold text-accent">
          Waitlist available. Checkout stays closed until fulfillment, policy,
          and support gates pass.
        </p>
      </div>
      <div className="grid gap-4 sm:grid-cols-3">
        {paidCloudTierOrder.map((tier) => {
          const plan = paidCloudPlans.find(
            (candidate) => candidate.tier === tier
          )!;
          const price = formatCloudPrice(
            vm.billingInterval === "monthly"
              ? plan.monthlyCents
              : plan.annualCents,
            vm.billingInterval
          );
      ...
      <p className="mt-4 max-w-2xl text-xs leading-6 text-muted">
        Cloud plan pricing is shown here for reference while OmniLux is in beta.
        Recurring cloud checkout is not open yet; joining the waitlist records
        which tier and billing interval you want when public cloud subscriptions
        open. Existing paid customers can still use Stripe-hosted plan
        management and the{" "}
        <a
          href={buildMarketingHref("/terms#billing-and-refunds")}
        ...
        <a
          href={buildMarketingHref("/privacy")}
          className="text-accent hover:underline"
        >
          Privacy Policy
        </a>
