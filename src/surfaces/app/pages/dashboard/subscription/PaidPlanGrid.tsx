  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="text-xs font-semibold text-accent">Annual saves {annualDiscountPercent}%</p>
      </div>
        ...
        {paidCloudTierOrder.map((tier) => {
          ...
          return (
            <div key={tier} className="rounded-xl surface-soft p-4">
              ...
              <button
                type="button"
                onClick={() => void vm.actions.startCheckout(tier)}
                disabled={!vm.user || vm.checkoutTier !== null}
              ...
              >
                {vm.checkoutTier === tier ? 'Opening checkout...' : `Start ${plan.name}`}
              </button>
            </div>
          );
        })}
      ...
      <p className="mt-4 max-w-2xl text-xs leading-6 text-muted">
        Cloud plan pricing is shown here for reference and checkout uses Stripe-hosted billing. Plan management and
        cancellations use the Stripe customer portal and follow the{' '}
