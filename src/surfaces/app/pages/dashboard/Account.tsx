...
export const Account = () => {
  const { provider, signIn, user } = useAuth();
  ...
  const handleProfileSave = async (event: FormEvent) => {
    ...
    if (usesWorkos) {
      setSaving(false);
      setMessage('Profile changes are managed by OmniLux sign-in.');
      return;
    }
  ...
  const handlePasswordChange = async (event: FormEvent) => {
    ...
    if (usesWorkos) {
      await signIn({ returnTo: '/dashboard/account', loginHint: user?.email });
      return;
    }
      ...
      <div className="mx-auto max-w-4xl space-y-8">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-muted">
            Manage the identity attached to your OmniLux Cloud account, along with password security and the current
            access state for managed media and self-hosted remote access.
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-xl surface-soft p-6">
            ...
            <form onSubmit={handleProfileSave} className="mt-4 space-y-4">
              ...
              >
                {usesWorkos ? 'Managed by sign-in' : 'Save profile'}
              </button>
            </form>
          </section>
          ...
          <section className="rounded-xl border border-border bg-background p-6">
            ...
            <dl className="mt-4 space-y-4 text-sm">
              <div>
                <dt className="text-xs font-semibold text-muted">Managed media</dt>
                <dd className="mt-1 text-foreground">
                  {accessProfile?.managedMediaEntitled ? 'Enabled for this account' : 'Not currently enabled'}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-muted">Relay access</dt>
                <dd className="mt-1 text-foreground">
                  {accessProfile?.relayRemoteAccessEntitled ? 'Included for this account' : 'Requires an eligible cloud plan'}
                </dd>
              ...
              <div>
                <dt className="text-xs font-semibold text-muted">Operator access</dt>
                <dd className="mt-1 text-foreground">
                  {accessProfile?.isOperator ? 'Available through the separate ops console' : 'Standard customer access'}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-muted">Session expiry</dt>
                <dd className="mt-1 text-foreground">
        ...
        <section className="rounded-xl border border-border bg-background p-6">
          ...
          <form onSubmit={handlePasswordChange} className="mt-4 space-y-4">
            ...
            <button
              ...
              disabled={
                saving ||
                (!usesWorkos && (!newPassword || !passwordConfirmed || !passwordMeetsLength || !passwordHasLetter || !passwordHasNumber))
              }
              className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground transition-colors hover:bg-accent/90 disabled:opacity-50"
            >
              {usesWorkos ? 'Continue to sign-in' : 'Update password'}
            </button>
          </form>
        </section>
      </div>
